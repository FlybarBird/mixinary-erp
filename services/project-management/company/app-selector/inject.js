...
/**
 * Suite app selector — inject into Plane shell header (company overlay).
 * Paths stay under /project-management; other apps are same-domain suite routes.
 */
(function () {
  const BASE = (window.__MIXINARY_BASE_PATH__ || "/project-management").replace(/\/$/, "");
  const APPS = [
    { id: "landing", label: "Suite Home", href: "/apps" },
  ...
  function mount() {
    ...
    wrap.style.cssText = "position:fixed;top:10px;right:12px;z-index:9999;font:14px/1.2 system-ui,sans-serif";
